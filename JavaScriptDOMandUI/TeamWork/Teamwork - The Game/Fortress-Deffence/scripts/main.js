...
$(document).ready(function() {
    ...
    function startGame() {
        ...
        for (i = MAX_PROJECTILES - 1; i >= 0; i--) {
            newProjectile = new Projectile(70, 300, projectileLayer, stage);
    ...
    function draw() {
        ...
        if (time > 50 && time % 10 === 0) {
            projectiles.forEach(function(proj) {
                if (!proj.isActive) {
                    var angle = getRandomInt(10, 60),
                        power = getRandomInt(40, 80);

                    angle *= Math.PI / 180;
                    proj.reset(angle, power);
                }
            })
    ...
    function update() {
        ...
        for (projCount = MAX_PROJECTILES - 1; projCount >= 0; projCount -= 1) {
            if (projectiles[projCount].isActive) { // Removes blown projectiles
                for (shipCount = ships.length - 1; shipCount >= 0; shipCount -= 1) {
                    if (ships[shipCount].isDestroyed) { // Removes destroyed ships
                        ...
                        if (doObjectsCollide(projectiles[projCount], ships[shipCount])) {
                            ...
                            //projectiles[projCount].isActive = false;
